...
class SubjectivityBasic {

    File mPolarityDir;
    String[] mCategories;
    DynamicLMClassifier<NGramProcessLM> mClassifier;

    public SubjectivityBasic(String[] args) {
        System.out.println("\nBASIC SUBJECTIVITY DEMO");
        mPolarityDir = new File(PolarityHierarchical.SUBJECTIVITY_MODEL);
        System.out.println("\nData Directory=" + mPolarityDir);
        mCategories = ["plot", "quote"];
        int nGram = 8;
    ...
    private void evaluate() throws IOException {
        ...
        JointClassifierEvaluator<CharSequence> evaluator = new JointClassifierEvaluator<CharSequence>(mClassifier, mCategories,storeInputs);
        System.out.println("\nEvaluating.");
        for (int i = 0; i < mCategories.length; ++i) {
            String category = mCategories[i];
            Classification classification = new Classification(category);
            ...
            for (int j = numTraining; j < sentences.length; ++j) {
                Classified<CharSequence> classified = new Classified<CharSequence>(sentences[j],classification);
